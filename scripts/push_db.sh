...
function push_mysql_ssh() {
    # colon is remote directory
    scp -P $REMOTE_SSH_PORT "${BACKUP_DB_PATH}.gz"  $REMOTE_SSH_LOGIN:"${BACKUP_DB_PATH}.gz"
}

function restore_remote_from_local_mysql() {
    ssh $REMOTE_SSH_LOGIN -p $REMOTE_SSH_PORT "$DB_ZCAT_CMD "${BACKUP_DB_PATH}.gz" | $REMOTE_MYSQL_CMD $REMOTE_DB_CREDS"
    echo "*** Restored remote database from ${BACKUP_DB_PATH}.gz"
}
